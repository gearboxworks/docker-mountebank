...
gb_clean() {
	...
	do
		...
		${LAUNCHBIN} manage clean "${GB_NAME}:${GB_VERSION}"
		RETURN="$?"
		if [ "${RETURN}" != "0" ]
		then
...
gb_list() {
	...
	${LAUNCHBIN} manage list "${GB_NAME}:${GB_VERSION}"
	RETURN="$?"
	return ${RETURN}
}
...
gb_rm() {
	...
	do
		...
		${LAUNCHBIN} manage uninstall "${GB_NAME}:${GB_VERSION}"
		RETURN="$?"
		if [ "${RETURN}" != "0" ]
		then
			EXIT="1"
...
gb_bash() {
	...
	do
		...
		${LAUNCHBIN} manage start "${GB_NAME}:${GB_VERSION}"
		RETURN="$?"
		if [ "${RETURN}" != "0" ]
		then
			EXIT="1"
...
gb_stop() {
	...
	do
		...
		${LAUNCHBIN} manage stop "${GB_NAME}:${GB_VERSION}"
		RETURN="$?"
		if [ "${RETURN}" != "0" ]
		then
			EXIT="1"
